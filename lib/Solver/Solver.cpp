...
bool Solver::getValue(const Query& query, ref<ConstantExpr> &result) {
  ...
  //klee_warning("#####################solver:getValue##################");
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(query.expr)) {
    //klee_warning("solver::getValue:dyn_cast success");
    result = CE;
    return true;
  }
  //klee_warning("solver::getValue:dyn_cast failed");
  // FIXME: Push ConstantExpr requirement down.
  ref<Expr> tmp;
  if (!impl->computeValue(query, tmp))
  {   
    //klee_warning("solver::getValue:impl->computeValue failed");
    return false;
  }
  result = cast<ConstantExpr>(tmp);
  
  //klee_warning("solver::getValue:impl->computeValue success");
  return true;
}
