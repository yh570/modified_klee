...
bool AddressSpace::resolveOne(const ref<ConstantExpr> &addr, 
  ...
  MemoryObject hack(address); // initialize a memoryobject as address

  if (const MemoryMap::value_type *res = objects.lookup_previous(&hack)) { /* ????  */
    if(c_flag)
      klee_warning("====resolveone: lookup blablabla====");
...
bool AddressSpace::resolveOne(ExecutionState &state,
                              ...
                              bool &success,
           		      bool c_flag) {
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(address)) {/* check the address could be a constant */
    if(c_flag)
      klee_warning("====resolveone: dyn_cast sucess====");
    success = resolveOne(CE, result, c_flag);/* check constant address  */
    if(c_flag){
      if(success)
	klee_warning("====resolveone: success is True");
      else
	klee_warning("====resolveone: success is False");
    }
    return true;
  } else {/* the majority of load go to this branch  */
    TimerStatIncrementer timer(stats::resolveTime);
    if(c_flag)
      klee_warning("====resolveone: dyn_cast failed====");
    ...
    if (!solver->getValue(state, address, cex, c_flag)) // --------------------mark1--------------------
      return false;
    else
      if(c_flag)
        klee_warning("====resolveone: solver get Value success====");

    uint64_t example = cex->getZExtValue();
    MemoryObject hack(example);
    const MemoryMap::value_type *res = objects.lookup_previous(&hack);// ---------------------mark2-----------------
    
    if (res) {
      if(c_flag)
        klee_warning("====resolveone: res == True====");
      const MemoryObject *mo = res->first;
      if (example - mo->address < mo->size) {
	if(c_flag)
          klee_warning("====resolveone: exampe - mo->address < mo->size");
        result = *res;
        success = true;
        return true;
      }
	...
	if(c_flag)
      ...
      if(c_flag)
        klee_warning("====resolveone: res == False====");
    ...
    if(c_flag)
      klee_warning("====resolveone: branch no cheap====");
