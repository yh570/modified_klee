...
/*
  if (strcmp(f->getName().str().c_str(),"klee_mark") == 0)
        executeMemoryOperation(state, false, arguments[0], 0, ki);

    Value *One = llvm::ConstantInt::get(Type::getInt32Ty(context),1);
    AllocaInst *pa = new AllocaInst(LLVMContextImpl::Int32Ty, "buf", ki);
    StoreInst* newstore = new StoreInst(One, pa, ki);
    LoadInst* newload = new LoadInst(pa, "loadvalue", ki);
   }


  if (strcmp(f->getName().str().c_str(),"klee_mark_stop") == 0){
    klee_mark_flag = 0;
    klee_warning("XXXXXXXXXXXXXXXXXXXXXXXXXXXX klee_mark is stop XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
  }  
  if (strcmp(f->getName().str().c_str(),"klee_mark_start") == 0){
    klee_mark_flag = 1;
    klee_warning("XXXXXXXXXXXXXXXXXXXXXXXXXXXX %s is found XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n", f->getName().str().c_str());
  }
*/
  //klee_warning("function: %s", f->getName().str().c_str());
  // Check user defined function
  // Insert load before printf
...
/*
      ...
      for (unsigned i = 0; i < func_Args; i++) {
        std::string str;
        llvm::raw_string_ostream string_stream(str);
        (*fType1->getParamType(0)).print(string_stream);
...
*/
    //if (strcmp(f->getName().str().c_str(),"klee_mark") == 0)
	//klee_warning("%s is not Intrinsic function", f->getName().str().c_str());
...
//	klee_warning("Checking Arguments[0]!");
    ...
    case Intrinsic::vastart:  {
      //if (strcmp(f->getName().str().c_str(),"klee_mark") == 0)
	//klee_warning("%s is not vastart function", f->getName().str().c_str());
      StackFrame &sf = state.stack.back();
    ...
    if (!f->isVarArg()) { /*  Consume record belong to this branch  */
      //if (strcmp(f->getName().str().c_str(),"klee_mark") == 0)
	//executeMemoryOperation(state, false, arguments[0], 0, ki);
	//klee_warning("--pppp--------------------------------%s is !isVarArg functionoooooooooooooooooooooooo", f->getName().str().c_str());
	...
	terminateStateOnError(state, "calling function with too few arguments",
      ...
      for (unsigned i = funcArgs; i < callingArgs; i++) {
        ...
        if (WordSize == Expr::Int32) {
	  //klee_warning("%s is wordsize == expr::Int32", f->getName().str().c_str());
	  ...
	  //klee_warning("%s is wordsize != expr::Int32", f->getName().str().c_str());
	     ...
	     //klee_warning("%s is argWidth > expr::Int64", f->getName().str().c_str());
      ...
      if (mo) {
	//klee_warning("%s is mo", f->getName().str().c_str());
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  */
  ...
  case Instruction::Load: {
    ...
    //if(klee_mark_flag == 1)
    //  klee_warning("-----------------------load on------------------------");
    executeMemoryOperation(state, false, base, 0, ki);
    //if(klee_mark_flag == 1){
    //  klee_warning("-----------------------load off-----------------------");
    //  klee_mark_flag = 0;
    //}
...
void Executor::executeMemoryOperation(ExecutionState &state,
                                      bool isWrite,/* load = false  */
                                      ref<Expr> address, /* load = base */
                                      ref<Expr> value /* undef if read */, /* load = 0 */
                                      KInstruction *target /* undef if write *//* load = ki */) {
  Expr::Width type = (isWrite ? value->getWidth() : 
                     getWidthForLLVMType(target->inst->getType()));//load go getWidthForLLVM blabla
  //if (call_flag == 1){
  //  std::cout << "after get Type" << std::endl;
  //}
  unsigned bytes = Expr::getMinBytesForWidth(type);

  if (SimplifySymIndices) { /* default is false  */ 
    //if (klee_mark_flag == 1)
    //  klee_warning("SymplifySymIndices");
    if (!isa<ConstantExpr>(address)) {
      //if (klee_mark_flag == 1)
      //  klee_warning("!isa<ConstantExpr>(address)");
      address = state.constraints.simplifyExpr(address); // states
    }
    if (isWrite && !isa<ConstantExpr>(value)){
      //if (klee_mark_flag == 1)
        //klee_warning("isWrite && !isa<ConstantExpr>(value)");
  ...
  ObjectPair op;
  bool success;
  solver->setTimeout(coreSolverTimeout); // solver
  bool c_flag = false;
  //if (klee_mark_flag == 1)
  //  c_flag = true;
  if (!state.addressSpace.resolveOne(state, solver, address, op, success, c_flag)) {
  
    //if (klee_mark_flag == 1)
      //klee_warning("!state.addressSpace.resolveOne");
    address = toConstant(state, address, "resolveOne failure");
    success = state.addressSpace.resolveOne(cast<ConstantExpr>(address), op, c_flag);// successful resolve
  }
  //else{
  //if (klee_mark_flag == 1)
  //    klee_warning("state.addressSpace.resolveOne == True");
  //}
  solver->setTimeout(0);

  if (0 && success) {
    if (klee_mark_flag == 1)
      klee_warning("success");
    const MemoryObject *mo = op.first;

    if (MaxSymArraySize && mo->size>=MaxSymArraySize) {
      //if (klee_mark_flag == 1)
      //  klee_warning("MaxSymArraySize");
    ...
    if (inBounds) {
      ...
      if (isWrite) {
        ...
        if (klee_mark_flag == 1)
          klee_warning("not write");
        ref<Expr> result = os->read(offset, type);
        
        if (interpreterOpts.MakeConcreteSymbolic){
          if (klee_mark_flag == 1)
            klee_warning("interpreterOpts.MakeConcreteSymbolic");
  ...
  bool incomplete = state.addressSpace.resolve(state, solver, address, rl,
                                               0, coreSolverTimeout);
  if (klee_mark_flag == 1)  
    klee_warning(incomplete ? "incomplete: True" : "incomplete: False");
  solver->setTimeout(0);
