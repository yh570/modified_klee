...
void Executor::executeCall(ExecutionState &state, 
  ...
  //klee_warning("JUST FOR TEST: %s found!", f->getName().str().c_str());
  if (f && f->isDeclaration()) {
   // klee_warning("JUST FOR TEST: %s found!", f->getName().str().c_str());
    ...
    //klee_warning("JUST FOR TEST: not_intrinsic %s found!", f->getName().str().c_str());  
    ...
    case Intrinsic::vastart:  {
      //klee_warning("JUST FOR TEST: Intrinsic %s found!", f->getName().str().c_str());
      StackFrame &sf = state.stack.back();
    ...
    if (!f->isVarArg()) {
      if (strcmp(f->getName().str().c_str(),"parse_record") == 0) {
        klee_warning("parse_record found");
        executeMemoryOperation(state, false, arguments[0], 0, ki);
      }
